...
declare const config: {
	...
	listenChange(key: string, onChange: (value: string) => void): void;
};
...
declare function renderMusicList(
	files: string[],
	args: {
		uniqueId: string;
		dontRenderBeforeLoaded?: boolean;
		errorText?: string;
		menuItems: object[];
		musicListInfo?: any;
		force?: boolean;
...
declare function getCurrentSelected(): any[];
...
declare class ContextMenu {
	constructor(menus: object[]);
	popup(pos: number[]): void;
}
...
SettingsPage.data.push(
	{ type: 'title', text: '网易云 NodeJS API 扩展' },
	{
		type: 'input',
		text: 'API 地址',
		description: '必填，无需最后的斜线（示例： https://api.example.com）。',
		configItem: 'ext.ncm.apiEndpoint'
	},
	{
		type: 'input',
		text: '要发送给 API 的 Header 信息',
		description: '选填，支持多个（格式：a=b&c=d，需要 URL 转义）。',
		configItem: 'ext.ncm.apiHeaders'
	},
	{
		type: 'input',
		inputType: 'number',
		text: '搜索时每页歌曲数量',
		description: '必填，默认为 30，推荐不超过 50，不能超过 100。',
		configItem: 'ext.ncm.searchLimit'
	},
	{
		type: 'boolean',
		text: '过滤无效歌曲',
	...
	},
	{
		type: 'boolean',
		text: '启用自动缓存',
		description: '开启后可提升歌单中网易云曲目的加载速度，但会占用更多内存空间。',
		configItem: 'ext.ncm.cacheEnabled'
	},
	{
		type: 'input',
		inputType: 'number',
		text: '最大缓存歌曲数量',
		description: '必填，默认为 50 首，缓存超出部分会自动删除。',
		configItem: 'ext.ncm.maxCacheCount'
	},
	{
		type: 'button',
		text: '清除缓存数据',
	...
	},
	{
		type: 'boolean',
		text: '自动格式化歌词',
		description: '开启后将会自动在网易云歌曲歌词中的无尾随空格英文标点后添加空格，增加 UI 美观度。',
		configItem: 'ext.ncm.formatLrc'
	},
	{
		type: 'input',
		inputType: 'number',
		text: '歌单信息最大并行请求数量',
		description: '必填，默认为 8，推荐不超过 16。',
		configItem: 'ext.ncm.maxParallelCount'
	}
);
...
async function request(path: string, query: object = {}) {
	let formatted = Object.keys(query)
		.map(k => encodeURI(k) + '=' + encodeURI(query[k]))
		.join('&');
	...
	if (headersConf) {
		headersConf
			...
			.map(it => it.split('='))
			.forEach(it => {
				const key = decodeURIComponent(decodeURI(it[0]));
				if (key.toLowerCase() == 'cookie') {
					formatted += '&cookie=' + it[1];
					return;
				}

				headers[key] = decodeURIComponent(decodeURI(it[1]));
			});
	}
	...
	for (let idsArr of split) {
		pool.submit(async () => {
			...
			resp.privileges.forEach((pri: any) => {
				playableMap[pri.id] = pri.plLevel != 'none' || pri.payed === 1 || pri.chargeInfoList.some((it: any) => it.chargeType == 0);
			});
				...
				menu: [
					{
						...
						content: {
							label: '复制单曲 ID',
							click: () => navigator.clipboard.writeText(getCurrentSelected()[0].substring(4))
						}
					},
					DownloadController.getMenuItems()
				]
			};
		} catch (err) {
			console.error(err);
		...
		renderList(container: HTMLElement) {
						...
						},
						{
							...
							click() {
								confirm(`确认移除网易云歌单 ${entry.name} 吗？`, () => {
									...
									config.setItem(
										'ext.ncm.musicList',
										currentList.filter(it => it.id != entry.id)
									);
			...
			if (errList.length) {
				alert('以下歌单因技术升级已被移除，对您造成的不便敬请谅解: ' + errList.map(it => it.name).join('，'), () => {
					...
					config.setItem(
						'ext.ncm.musicList',
						oldList.filter(it => !removedIds.includes(it.id))
					);
				});
			}
		},
